...
class Game:
    def __init__(self, canvas, x_field_size: int, y_field_size: int):
        self.__canvas = canvas
        self.__draw_canvas = Draw(self.__canvas)
    ...
    def __init_images(self):
        ...
        self.__images = {
            CheckerType.WHITE_REGULAR: Image.open(Path('assets', 'white-regular.png')).resize((CELL_SIZE, CELL_SIZE), Image.ANTIALIAS),
            CheckerType.BLACK_REGULAR: Image.open(Path('assets', 'black-regular.png')).resize((CELL_SIZE, CELL_SIZE), Image.ANTIALIAS),
            CheckerType.WHITE_QUEEN: Image.open(Path('assets', 'white-queen.png')).resize((CELL_SIZE, CELL_SIZE), Image.ANTIALIAS),
            CheckerType.BLACK_QUEEN: Image.open(Path('assets', 'black-queen.png')).resize((CELL_SIZE, CELL_SIZE), Image.ANTIALIAS),
        }
    ...
    def __animate_move(self, move: Move):
        ...
        self.__canvas.move(animated_checker, dx, dy)
    ...
    def __draw_field_grid(self):
        ...
        for y in range(self.__field.y_size):
            for x in range(self.__field.x_size):
                self.__draw_canvas.rectangle([x * CELL_SIZE, y * CELL_SIZE, x * CELL_SIZE + CELL_SIZE, y * CELL_SIZE + CELL_SIZE], fill=FIELD_COLORS[(y + x) % 2])

                # Отрисовка рамок у необходимых клеток
                if (x == self.__selected_cell.x and y == self.__selected_cell.y):
                    for i in range(1, 5):
                        self.__draw_canvas.rectangle([(x * CELL_SIZE) + i, (y * CELL_SIZE) + i, (x * CELL_SIZE + CELL_SIZE) - i, (y * CELL_SIZE + CELL_SIZE) - i], fill=None, outline=SELECT_BORDER_COLOR)
                elif (x == self.__hovered_cell.x and y == self.__hovered_cell.y):
                    self.__draw_canvas.rectangle([x * CELL_SIZE + BORDER_WIDTH // 2, y * CELL_SIZE + BORDER_WIDTH // 2, x * CELL_SIZE + CELL_SIZE - BORDER_WIDTH // 2, y * CELL_SIZE + CELL_SIZE - BORDER_WIDTH // 2], fill=None, outline=HOVER_BORDER_COLOR)
                ...
                if (self.__selected_cell):
                    ...
                    for move in player_moves_list:
                        if (self.__selected_cell.x == move.from_x and self.__selected_cell.y == move.from_y):
                            self.__draw_canvas.ellipse([move.to_x * CELL_SIZE + CELL_SIZE / 3, move.to_y * CELL_SIZE + CELL_SIZE / 3, move.to_x * CELL_SIZE + (CELL_SIZE - CELL_SIZE / 3), move.to_y * CELL_SIZE + (CELL_SIZE - CELL_SIZE / 3)], fill=POSIBLE_MOVE_CIRCLE_COLOR)
        ...
        for y in range(self.__field.y_size):
            for x in range(self.__field.x_size):
                ...
                if (self.__field.type_at(x, y) != CheckerType.NONE and not (x == self.__animated_cell.x and y == self.__animated_cell.y)):
                    self.__canvas.paste(self.__images.get(self.__field.type_at(x, y)), (x * CELL_SIZE, y * CELL_SIZE), self.__images.get(self.__field.type_at(x, y)))

    def __select_checker(self, x: int, y: int):
